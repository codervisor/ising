...
# Spec 047: Scoring Formula Redesign — Post-046 Analysis

## Context

After implementing spec 046 (boundary-aware analysis) and running the full OSS benchmark,
a detailed analysis of the scoring formula revealed structural weaknesses that incremental
fixes cannot resolve. This spec documents the findings and proposes next steps.

### Benchmark Results (Spec 046, 2026-04-03)

14 of 28 repos analyzed (14 FAIL from pre-existing FK constraint DB bug, unrelated to 046).
...
### Problem 1: Multiplicative x Subtractive Mixing
...
The sigmoid `0.25 x x/(x+3)` saturates quickly:
...
A 19x difference in signal count produces a 6% difference in penalty. The sigmoid
...
```
cycles=4x, god_modules=3x, ticking_bombs=3x, fragile_boundaries=2x,
unstable_deps=2x, shotgun_surgery=1.5x, ghost_coupling=1x, systemic=2.5x
```
...
The core risk model:
```
change_load = normalize(change_freq x churn_rate)
...
```
burden = complexity x 0.4 + instability x 0.3 + coupling x 0.3
capacity = max(1.0 - burden, 0.05)
```

The 0.4/0.3/0.3 weights have never been validated against defect data. The instability
metric `fan_out / (fan_in + fan_out)` is Robert Martin's SDP definition designed for
package-level analysis, not file-level. A leaf module with zero imports gets instability=0
regardless of actual volatility.
...
```
zone_sub_score = raw x (N/50) + 0.75 x (1 - N/50)    for N < 50
```
...
These design choices are sound and should be preserved:

1. **Multi-layer signal detection**: comparing structural, temporal, and defect layers to
   find anomalies. Ghost coupling and unnecessary abstraction are real phenomena.

2. **Boundary-aware propagation attenuation** (spec 046): module boundaries as risk
   firewalls matches how well-designed software behaves. Django's B->A validates this.

3. **Sigmoid saturation on signal penalty**: prevents any single metric from dominating.
   The issue is the saturation point, not the saturation itself.

4. **Median over mean**: correctly resists outlier domination in small repos.

5. **sqrt(N) normalization**: sub-linear scaling is the right choice for cross-repo
   comparability.

6. **Per-file safety factors and hotspot ranking**: consistently accurate across all
   validation rounds. These are the actionable outputs.

7. **Percentile-based risk tiers**: honestly self-referential, don't pretend absolute.

---

## Proposed Changes

### Phase 1: Fix the DB FK Constraint Bug

**Goal**: Get all 28 benchmark repos passing.

**Priority**: P0 -- we cannot validate key predictions (transformers D->C, vllm D->C,
ha-core D->C) without this fix. The FK constraint bug predates spec 046 and blocks 14
repos.

### Phase 2: Separate Defect Churn from Feature Churn

**Goal**: Stop treating all changes as equal risk. This is the single most impactful
improvement possible.

**Approach**: Use git commit message heuristics to classify commits:
- **Fix commits**: messages containing fix, bug, patch, revert, hotfix, issue, CVE,
  security, crash, error, broken, regression
- **Feature commits**: messages containing feat, add, implement, new, support, enable
- **Refactor commits**: messages containing refactor, cleanup, rename, reorganize,
  simplify, extract, move
- **Maintenance commits**: messages containing update, bump, upgrade, dependency, version,
  chore, ci, docs
- **Unknown**: everything else (treated as mixed by default)

Compute two separate change metrics per module:
```
defect_churn = sum_over_fix_commits(lines_changed_in_module)
feature_churn = sum_over_feature_commits(lines_changed_in_module)
```

Redefine change_load:
```
change_load = normalize(defect_churn x 3.0 + feature_churn x 1.0)
```

Feature churn still contributes (actively developed code is still under pressure) but
at reduced weight. Defect churn is the true risk indicator.

**Validation target**: transformers and vllm should improve from D to C or better.
Active ML development is feature-heavy; the current D rating conflates velocity with risk.

### Phase 3: Fix the Formula Structure

**Goal**: Eliminate multiplicative x subtractive mixing.

**Proposal**: Fully multiplicative with signal as a discount factor:
```
score = zone_sub_score x coupling_modifier x containment_modifier x signal_factor
```
Where:
```
...
This makes the signal penalty proportional: a 15% penalty reduces any pre-penalty
score by 15%, regardless of the base value.

**Validation**: Re-run benchmark. Verify gin >= B, flask stays C, django stays A.
Grade thresholds may need recalibration -- expect a 1-2 grade shift when the formula
becomes fully multiplicative.

### Phase 4: Widen Modifier Ranges

**Coupling modifier**: Expand from [0.95, 1.03] to [0.85, 1.05]. This gives
structural coupling a 20% total swing -- enough to shift a grade.
...
**Constraint**: Change one at a time, benchmark between changes. Compound effects
are unpredictable.
...
**Proposal**: Replace `0.25 x x/(x+3)` with a piecewise curve that has more
dynamic range above x=5:
```
if x <= 5:    penalty = 0.25 x x / (x + 3)         (same as now)
if x > 5:     penalty = 0.156 + 0.094 x log2(x/5)  (log growth above)
               capped at 0.30
```

This keeps the gentle start but allows more discrimination for signal-heavy repos.
kubernetes (x~11.4) would get penalty~0.24 instead of 0.19.

**Risk**: Raising the penalty cap from 0.25 to 0.30 could drop some repos a grade.

### Phase 6: Empirical Signal Weight Calibration
...
2. Look up external quality proxies: GitHub issue counts, CVE counts, contributor
...
rate. Adjust weights to maximize correlation between signal score and issue rate.

**Known limitation**: This is a research task, not an engineering task. Defer until
defect data is available from Phase 2 (commit message classification).
...
|-------|--------|--------|----------|
| Phase 1 (FK bug) | Low | High (unblocks validation) | **P0** |
| Phase 2 (defect vs feature churn) | Medium | **Critical** (fixes root cause) | **P1** |
| Phase 3 (formula structure) | Low | Medium (cleaner semantics) | P1 |
| Phase 4 (widen ranges) | Low | Low-Medium | P2 |
| Phase 5 (penalty curve) | Low | Medium | P2 |
| Phase 6 (signal weight calibration) | High (research) | Medium | P3 |

Phase 2 is the most impactful single change. Phase 1 is prerequisite for validating
anything on D-grade repos. Phase 3 is a clean-up that should happen before further
formula changes.
...
- [ ] Formula uses consistent combination semantics (not mixed mult/sub)
- [ ] Coupling and containment modifiers have measurable grade impact (>0.05 on >=3 repos)
...
- [ ] Signal penalty discriminates between prometheus (1.7K) and kubernetes (33K signals)
...
- **No cross-repo calibration.** Grades are repo-internal assessments. Comparing
  ...
  which requires external ground truth we don't have.
- **No new signal types.** Signal detection improvements belong in signal-specific
  specs. This spec only changes how existing signals feed into the health score.

## Honest Assessment of What We Measure

**Grade A means**: "Low median defect-related change-risk, few detected cross-boundary
architectural signals relative to codebase size, good boundary containment."

**Grade A does NOT mean**: "Good codebase," "low defect rate," or "well-architected."
Those claims require defect histories, maintenance cost data, or developer surveys that
this tool does not have.

The tool measures a narrow slice of code health -- structural properties visible from
AST + git history. It's useful for identifying hotspots and architectural anti-patterns,
but it cannot replace expert judgment about code quality.
