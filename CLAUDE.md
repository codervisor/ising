...
- **Run every formula change against the 12-repo test set** in `/tmp/oss-repos/` (fastapi, gin, django-rest-framework, ollama, vllm, transformers, open-webui, langchain, llama.cpp, ha-core, grafana, odoo). Check the output table. If gin gets below B or odoo gets A with no caveats, something is wrong.
